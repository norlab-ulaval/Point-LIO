...
deque<sensor_msgs::msg::Imu> imu_deque;
...
sensor_msgs::msg::Imu imu_last_ptr;
...
void imu_cbk(const sensor_msgs::msg::Imu& msg_in)
{
    publish_count++;
    sensor_msgs::msg::Imu msg = msg_in;

    msg.header.stamp = rclcpp::Time((rclcpp::Time(msg_in.header.stamp).seconds() + time_lag_imu_to_lidar) * 1e9);
    ...
    {
        RCLCPP_ERROR(node->get_logger(), "imu loop back, clear deque");
...
void publish_odometry(rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped)
{
    ...
    {
        odomAftMapped.header.stamp = rclcpp::Time(time_current * 1e9);
    }
    else
    {
...
int main(int argc, char** argv)
{
    ...
    node = std::make_shared<LaserMappingNode>();
    tfBroadcaster = std::unique_ptr<tf2_ros::TransformBroadcaster>(new tf2_ros::TransformBroadcaster(*node));

    p_imu = std::make_shared<ImuProcess>(node);
    readParameters(node);
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudEffect = node->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_effected", 100000);
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudMap = node->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", 100000);
    rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped = node->create_publisher<nav_msgs::msg::Odometry>("/aft_mapped_to_init", 100000);
...
//------------------------------------------------------------------------------------------------------
 ...
 << " " << state_out.omg.transpose() << " " << state_out.acc.transpose() << " " << state_out.gravity.transpose() << " " << state_out.bg.transpose() << " "
                                     << state_out.ba.transpose() << " " << feats_undistort->points.size() << endl;
                        }
                    }
                ...
                for(k = 0; k < time_seq.size(); k++)
                {
                    ...
                    if(publish_odometry_without_downsample)
                    {
                        ...
                        if(runtime_pos_log)
                        {
                            ...
                            fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " " << euler_cur.transpose() << " " << state_in.pos.transpose() << " "
