...
void Preprocess::process(const livox_ros_driver::msg::CustomMsg& msg, PointCloudXYZI::Ptr& pcl_out)
{
    avia_handler(msg);
    *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const livox_ros_driver::msg::CustomMsg& msg)
{
    pl_surf.clear();
    // pl_corn.clear();
    pl_full.clear();
    ...
    for(uint i = 1; i < plsize; i++)
    {
        if((msg.points[i].line < N_SCANS) && ((msg.points[i].tag & 0x30) == 0x10 || (msg.points[i].tag & 0x30) == 0x00))
        {
            ...
            if(valid_num % point_filter_num == 0)
            {
                ...
                { pl_full[i].curvature = fabs(pl_full[i].curvature - pl_full[i - 1].curvature) < 1.0 ? pl_full[i].curvature : pl_full[i - 1].curvature + 0.004166667f; }
...
void Preprocess::rslidar_handler(const sensor_msgs::msg::PointCloud2& msg)
{
    ...
    for(int i = 0; i < plsize; i++)
    {
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.curvature = pl_orig.points[i].t * time_unit_scale;  // curvature unit: ms // cout<<added_pt.curvature<<endl;
        if(!given_offset_time)
        {
            int layer = pl_orig.points[i].ring;
            double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;
